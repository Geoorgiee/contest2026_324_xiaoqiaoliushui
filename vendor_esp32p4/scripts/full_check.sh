...
check_documentation() {
    ...
    # 检查参考工程分析
    local reference_dir="$PROJECT_ROOT/docs/reference"
    ...
    # 检查改进总结
    local improvement_summary="$PROJECT_ROOT/docs/IMPROVEMENT_SUMMARY.md"
...
check_script_permissions() {
    ...
    for script in build.sh flash.sh link_drivers.sh collect_evidence.sh validate_defconfig.sh full_check.sh; do
...
check_skills() {
    ...
    if [ -d "$skills_dir" ]; then
        ...
        print_check "PASS" "skill 目录存在 ($skill_count 个 skill)"
    else
        print_check "WARN" "skill 目录不存在"
    fi
}
...
generate_report() {
    ...
    if [ $CHECK_FAIL -gt 0 ]; then
        echo -e "  1. 修复所有失败的检查项"
        echo -e "  2. 重新运行自检: ./vendor_esp32p4/scripts/full_check.sh"
    fi
...
# 主函数
main() {
    echo -e "${BLUE}ESP32-P4 工程完整自检${NC}"
    echo -e "${BLUE}========================================${NC}"
