...
/* getreg32/putreg32/modifyreg32 -> riscv_internal.h (openvela NuttX) */
 ...
 * NOTE: 0x1c/0x80/0x88 follow the ESP-IDF register map
 * (components/soc/esp32p4/register/mipi_dsi_bridge_reg.h); the original
 * vendor_esp32p4 driver used pre-release offsets (0x130/0x144).
 */
...
#define BRG_DMA_BLK_INTERVAL_REG 0x01c
...
#define BRG_HOST_CTRL_REG       0x080
#define BRG_DMA_FLOW_CTRL_REG   0x088
...
static int  dsi_pandisplay(FAR struct fb_vtable_s *vtable,
                             FAR struct fb_planeinfo_s *pinfo);
...
static void dsi_bridge_init(FAR struct esp32p4_dsi_priv_s *priv)
{
   ...
   * ESP-IDF register map: DMA_BLOCK_INTERVAL_EN is bit 29 (bitpos [29]).
   */

  dsi_brg_setbits((1 << 29), BRG_DMA_BLK_INTERVAL_REG);
...
/* NOTE: an fb blank hook is not available in the openvela fb_vtable_s;
 * DCS display on/off (0x28/0x29) is performed by the panel init sequence
 * and the board backlight/backlight-stub call path.
 ****************************************************************************/
  ...
  FAR const struct esp32p4_lcd_panel_config_s *panel_config)
{
  ...
  /* NOTE: openvela NuttX fb_vtable_s has no 'blank' member; blanking via
   * DCS 0x28/0x29 moves to the board backlight hook / esp32p4_lcd.c. */
